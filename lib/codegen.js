 ...
 * @param  {Array}         config.excluded_operations List of operationIds to exclude from generation.
 * @return {Promise}
 */
codegen.generate = config => new Promise((resolve, reject) => {
  ...
  swagger2(config.swagger).then(swagger => {
    ...
    // Filter out excluded operationIds
    if (config.excluded_operations && config.excluded_operations.length > 0) {
      const excluded = new Set(config.excluded_operations);
      
      // Filter operations in the paths based on operationId
      _.forEach(config.swagger.paths, (pathItem, pathName) => {
        _.forEach(pathItem, (operation, method) => {
          if (method !== 'endpointName' && excluded.has(operation.operationId)) {
